...
namespace TouchpointMedical.Integration.PointClickCare.Services.Background
{
        ...
        IServiceScopeFactory scopeFactory) : BackgroundService
    {
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            while (_touchpointOptions.Enabled && !stoppingToken.IsCancellationRequested)
            {
                _logger.LogDebug("PointClickCareRedisDebounceWorker Heartbeat");
                ...
                var dueAt = nowMs + _touchpointOptions.WindowMs;

                // pull up to 100 patients whose due time <= now
                var ids = await db.SortedSetRangeByScoreAsync($"{_touchpointOptions.KeyPrefix}:due", double.NegativeInfinity, nowMs, take: 100);

                if (ids.Length == 0)
                {
                    await Task.Delay(TimeSpan.FromMilliseconds(_touchpointOptions.ProcessorDelayMs), stoppingToken);
                ...
                foreach (var rv in ids)
                {
                    ...
                    if (!await db.StringSetAsync(
                        $"{_touchpointOptions.KeyPrefix}:lock:{pccKey}", 
                    ...
                    try
                    {
                        ...
                        var lastVal = await db.StringGetAsync($"{_touchpointOptions.KeyPrefix}:last:{pccKey}");
                        if (!lastVal.HasValue)
                        {
                            await db.SortedSetRemoveAsync(
                                $"{_touchpointOptions.KeyPrefix}:due", (string)pccKey);

                            continue;
                        }

                        var last = (long)lastVal;
                        var newDue = last + _touchpointOptions.WindowMs;
                        if (newDue > nowMs)
                        {
                            // someone clicked save again; push the reminder out and skip for now
                            await db.SortedSetAddAsync(
                                $"{_touchpointOptions.KeyPrefix}:due", (string)pccKey, newDue);
                        ...
                        var types = (await db.SetMembersAsync(
                            $"{_touchpointOptions.KeyPrefix}:types:{pccKey}"))
                        ...
                        {
                            await db.SortedSetAddAsync(
                                $"{_touchpointOptions.KeyPrefix}:error", 
            ...
            IList<string> notificationEvents)
        {
            ...
            try
            {
                if (notificationEvents != null)
                {
                    ...
                    string facilitySettingsKey = pccKey.ToString(PointClickCareKeyType.Facility);
                    ...
                    foreach (string eventItem in notificationEvents)
                    {
                        ...
                        await db.SetAddAsync(
                            $"{_touchpointOptions.KeyPrefix}:eventItem:try", $"{pccKey}:{eventGroup}:{eventItem}:{transactionId}");
                        ...
                        try
                        {
                            ...
                            await db.SetAddAsync(
                                $"{_touchpointOptions.KeyPrefix}:eventItem:success", $"{pccKey}:{eventGroup}:{eventItem}:{transactionId}");
                        ...
                        {
                            await db.SetAddAsync(
                                $"{_touchpointOptions.KeyPrefix}:eventItem:failed", $"{pccKey}:{eventGroup}:{eventItem}::{transactionId}:{ex.Message}");
